...
class DropSchemaDoctrineODMCommand extends DropCommand
{
    protected function configure()
    {
        ...
        $this
            ->setName('doctrine:riak:schema:drop')
            ->addOption('dm', null, InputOption::VALUE_REQUIRED, 'The document manager to use for this command.')
            ->setHelp(<<<EOT
The <info>doctrine:riak:schema:drop</info> command drops the default document manager's schema:

  <info>./app/console doctrine:riak:schema:drop</info>

You can also optionally specify the name of a document manager to drop the schema for:

  <info>./app/console doctrine:riak:schema:drop --dm=default</info>
EOT
        );
    }
